...
buttonOpen.addEventListener("click", function () {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${inputOpen.value}&units=metric&appid=b5393e979ce8fef443d695c2124eece5`
  )
    .then((data) => {
      return data.json();
    })
    .then((data) => {
      ...
      tempOpen.innerHTML = `Current temp = ${
        data.main.temp + Number(tempUp)
      }ºC`;
      feelOpen.innerHTML = `Feels like = ${
        data.main.feels_like + Number(tempUp)
      }ºC`;
    })
    .catch((error) => {
      console.log(error);
...
buttonApi.addEventListener("click", function () {
  fetch(
    `http://api.weatherapi.com/v1/current.json?key=1ba124c1851e401e9db94404232711&q=${inputApi.value}&aqi=no`
  )
    .then((data) => {
      return data.json();
    })
    .then((data) => {
      ...
      tempApi.innerHTML = `Current temp = ${
        data.current.temp_c + Number(tempUp)
      }ºC`;
      feelApi.innerHTML = `Feels like = ${
        data.current.feelslike_c + Number(tempUp)
      }ºC`;
    });
});
